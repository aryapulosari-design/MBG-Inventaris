...
<head>
    ...
    <title>Laporan Mutasi Transaksi MBG - {{ now()->format('d M Y') }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="header">
        <h1>LAPORAN MUTASI LOGISTIK (KELUAR-MASUK)</h1>
        <p>PROGRAM MAKAN BERGIZI GRATIS (MBG)</p>
        @if(!empty($filters))
            <p style="margin-top:5px; font-style: italic">
                Filter: 
                @if(!empty($filters['from'])) Dari: {{ $filters['from'] }} @endif
                @if(!empty($filters['to'])) Sampai: {{ $filters['to'] }} @endif
                @if(!empty($filters['type'])) Jenis: {{ $filters['type'] === 'in' ? 'Barang Masuk' : 'Barang Keluar' }} @endif
            </p>
        @endif
    </div>
            ...
            @foreach($data['rows'] as $row)
                <tr>
                    @foreach($row as $index => $cell)
                        @if($index === 3) {{-- Kolom Jenis --}}
                            <td class="{{ $cell === 'Masuk' ? 'badge-in' : 'badge-out' }}">{{ $cell }}</td>
                        @else
                            <td class="{{ in_array($index, [4,6,7]) ? 'text-right' : '' }}">{{ $cell }}</td>
                        @endif
                    @endforeach
                </tr>
            @endforeach
